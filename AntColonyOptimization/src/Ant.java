...
public class Ant implements Callable<Ant> {
    public static final double Q = 0.0005; //parameter used for adjusting the amount of pheromone deposited, the value is between 0 & 1
    public static final double RHO = 0.2; //parameter used for varying the level of pheromone evaporation, the value is between 0 & 1
    public static final double ALPHA = 0.01; //parameter used for controlling the importance of pheromone trail, the value is >= 0
    public static final double BETA = 9.5; //parameter used for controlling the importance of distance between source and destination
    ...
    private void adjustPhermoneLevel(int x, int y, double distance) {
        ...
        while (!flag) {
            ...
            double updatePhermoneLevel = (1-RHO)*currentPhermoneLevel + Q/distance;
            if (updatePhermoneLevel < 0.00) flag = aco.getPhermoneLevelsMatrix()[x][y].compareAndSet(0);
            else flag = aco.getPhermoneLevelsMatrix()[x][y].compareAndSet(updatePhermoneLevel);
        }
    }
